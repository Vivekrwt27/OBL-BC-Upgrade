report 50114 "Purchase Order GST kbs"
{
    DefaultLayout = RDLC;
    RDLCLayout = '.\ReportLayouts\PurchaseOrderGSTkbs.rdl';
    Caption = 'Order';
    ...
    dataset
    {
        dataitem("Purchase Header"; 38)
        {
            ...
            column(TaxperPur1; 0) // 16630  FORMAT(Purchline1."Tax %")
            {
            }
            column(App_Status; "Purchase Header"."Approval Status")
            {
            }
            ...
            column(Pur_status; "Purchase Header".Status)
            {
            }
            column(VEND_ADDRESS; 'M/s. ' + BuyFromAddr[1])
            {
            }
            column(Comment1; PurchCommLine.Comment)
            {
            }
            ...
            column(blnCrrExch; blnCrrExch)
            {
            }
            column(decExchngRate; decExchngRate)
            {
            }
            column(txtApproverName; txtApproverName)
            {
            }
            column(blnTermCond; blnTermCond)
            {
            }
            column(cdcurr; cdcurr)
            {
            }
            dataitem(CopyLoop; 2000000026)
            {
                ...
                dataitem(PageLoop; 2000000026)
                {
                    ...
                    {
                        DataItemTableView = SORTING(Number);
                        column(GrandlGstAmt; GrandlGstAmt)
                        {
                        }
                        column(SnoNew; SnoNew)
                        {
                        }
                        column(TotalGSTAmount_PurchaseLine; TotalAmt) // 16630 "Purchase Line"."Total GST Amount"
                        {
                        }
                        column(TotalGstAmt; TotalGstAmt)
                        {
                        }
                        column(Amount; TotalAmt) // 16630 "Purchase Line"."Total GST Amount"  "Purchase Line"."Line Amount"
                        {
                        }
                        column(HSNSACCode_PurchaseLine; "Purchase Line"."HSN/SAC Code")
                        {
                        }
                        column(GSTBaseAmt; "Purchase Line"."Line Amount") // 16630 "Purchase Line"."GST Base Amount"
                        {
                        }
                        column(GST_PurchaseLine; GSTper) // 16630 blank "Purchase Line"."GST %"
                        {
                        }
                        ...
                        column(KKCessAmt; 0) // 16630 blank "Purchase Line"."KK Cess Amount"
                        {
                        }
                        column(PossCenvat; "Purchase Line"."Possible Cenvat")
                        {
                        }
                        column(TDSAmt; 0) // 16630 blank PurchLine."Bal. TDS Including SHE CESS"
                        {
                        }
                        ...
                        column(PurchLineAmountToVendor; TotalAmt) // 16630 PurchLine."Amount To Vendor"
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        }
                        column(PurchLineExciseAmount; 0) // 16630 blank PurchLine."Excise Amount"
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        }
                        column(PurchLineTaxAmount; 0) // 16630 blank PurchLine."Tax Amount"
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        }
                        column(PurchLineServiceTaxAmount; 0) // 16630 blank  PurchLine."Service Tax Amount"
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        }
                        ...
                        column(PurchLineTotalTDSIncludingSheCess; 0) // 16630 blank -PurchLine."Total TDS Including SHE CESS"
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        }
                        column(PurchLineWorkTaxAmount; 0) // 16630 blank  -PurchLine."Work Tax Amount"
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        }
                        column(PurchLineSerTaxeCessAmt; 0) // 16630 blank PurchLine."Service Tax eCess Amount"
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        }
                        column(PurchLineSerTaxSHECessAmt; 0) // 16630 blank PurchLine."Service Tax SHE Cess Amount"
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        }
                        ...
                        column(ServTaxeSBCAmt; 0) // 16630 blank PurchLine."Service Tax SBC Amount"
                        {
                        }
                        ...
                        dataitem(DimensionLoop2; 2000000026)
                        {
                            ...
                            begin
                                ...
                                /*  //Raushan GST
                                 ...
                                 // 16630    DetailedGSTEntryBuffer.SETRANGE("Document Type", PurchLine."Document Type");
 ...
 DetailedGSTEntryBuffer.NEXT = 0; */
                        ...
                        begin
                            ...
                            if PurchLine.FindSet() then
                                repeat
                                    TotalAmt += PurchLine."Line Amount";
                                    GSTSetup.Get();
                                        ...
                                        if TaxTransactionValue.FindSet() then
                                            repeat
                                                case TaxTransactionValue."Value ID" of
                                                    ...
                                                    4:
                                                        begin
                                                            GSTBaseAmt += Abs((Round(TaxTransactionValue.Amount, GetGSTRoundingPrecision(ComponentName))));
                                ...
                                // TotalAmt += LineAmt + igst + cgst + sgst;
                                until PurchLine.Next() = 0;


                            IF TaxPer = '' THEN
                                /*   IF "Purchase Line"."Tax %" <> 0 THEN
                                       TaxPer := FORMAT("Purchase Line"."Tax %") + ' %';*/ // 16630
                            ...
                            TotalGstAmt += igst + cgst + sgst;


                            TotalGstAmt := Amttovendor2.TotalGSTAmtLine("Purchase Line");  //RKS 060717
                            ...
                            /*  TotalServiceTaxAmount += PurchLine."Service Tax Amount";
                              ...
                              TotalKKCessAmount += PurchLine."KK Cess Amount";*/ // 16630
                            ...
                            // 16630 TotalGSTAmount += PurchLine."Total GST Amount"; //GST
                            TotalAmount += TotalAmt; //16767
                        ...
                        begin
                            ...
                            /* CurrReport.CREATETOTALS(PurchLine."Line Amount", PurchLine."Inv. Discount Amount",
                             ...
                             CurrReport.CREATETOTALS(PurchLine."Service Tax SHE Cess Amount");*/ // 16630
                            CLEAR(TotLM);
                            CLEAR(TotalDisAmt);
                        end;
                    }
                    ...
                    dataitem(RowsIns; 2000000026)
                    {
                        ...
                        column(decTotAmount; decTotAmount)
                        {
                        }
                        column(AmountInWord; descri[1] + descri[2])
                        {
                        }
                        ...
                        begin
                            ...
                            descri[1] := 'XXXXXXXXXXX';
                            descri[2] := descri[1];
                            decTotAmount := TotLM + TotalDisAmt + ChargesAmount + FreightCharges + GrandlGstAmt;
                            decTotAmount := ROUND(decTotAmount / decExchngRate, 2);
                            rptCheck.InitTextVariable;
                            rptCheck.FormatNoText(descri, decTotAmount, "Purchase Header".Currency_Code);
                        end;
                ...
                begin
                    ...
                    // 16630  PurchPostPrepmt.BuildInvLineBuffer2("Purchase Header", PrepmtPurchLine, 0, PrepmtInvBuf);
            ...
            begin
                ...
                //FixedLenth:=0; //RKS
                //Keshav18042020
                IF "Purchase Header".Currency_Code <> '' THEN BEGIN
                    blnCrrExch := TRUE;
                    decExchngRate := "Purchase Header"."Exchange Rate";
                    cdcurr := "Purchase Header".Currency_Code;//KEshav05082020
                END ELSE BEGIN
                    blnCrrExch := FALSE;
                    decExchngRate := 1;
                    cdcurr := 'Rs.';//KEshav05082020
                END;
                IF "Purchase Header".Status = "Purchase Header".Status::Released THEN BEGIN
                    txtApproverName := '';
                    rApprovalEntry.RESET;
                    rApprovalEntry.SETRANGE("Document No.", "Purchase Header"."No.");
                    rApprovalEntry.SETRANGE("Document Type", "Purchase Header"."Document Type");
                    rApprovalEntry.SETRANGE(Status, rApprovalEntry.Status::Approved);
                    IF rApprovalEntry.FIND('+') THEN BEGIN
                        rUser.RESET;
                        rUser.SETRANGE("User Name", rApprovalEntry."Approver ID");
                        IF rUser.FIND('-') THEN BEGIN
                            txtApproverName := rUser."Full Name";
                        END;
                    END;
                END;
                //Keshav18042020
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field("Print Terms & Conditions"; blnTermCond)
                    {
                        ApplicationArea = All;
                    }
                }
        ...
        trigger OnOpenPage()
        begin
            ArchiveDocument := PurchSetup."Archive Orders"; // 16630 replace field "Archive Quotes and Orders"
    ...
    begin
        // IF blnTermCond=TRUE THEN BEGIN
        //  "Purchase Header".SETRECFILTER;
        //   REPORT.EXECUTE(50411,
        //     STRSUBSTNO('<?xml version="1.0" standalone="yes"?><ReportParameters name="Purchase_Order(T&amp;C)" id="50411"><Options>'+
        //                '<Field name="Remark1" /><Field name="Remark2" /><Field name="Remark3" /></Options><DataItems>'+
        //                '<DataItem name="Purchase Header">VERSION(1) SORTING(Field3,Field1) WHERE(Field3=1(%1))</DataItem>'+
        //                '<DataItem name="Purchase Line">VERSION(1) SORTING(Field3,Field4,Field1)</DataItem>'+
        //                '</DataItems></ReportParameters>',"Purchase Header"."No."));
        // END;
    end;
    ...
    var
        ...
        blnCrrExch: Boolean;
        decExchngRate: Decimal;
        txtApproverName: Text;
        rptCheck: Report "Check Report";
        descri: array[2] of Text;
        decTotAmount: Decimal;
        blnTermCond: Boolean;
        rCompInfo: Record "Company Information";
        cdcurr: Code[5];
        Amttovendor2: Codeunit CalcAmttoVendor;
