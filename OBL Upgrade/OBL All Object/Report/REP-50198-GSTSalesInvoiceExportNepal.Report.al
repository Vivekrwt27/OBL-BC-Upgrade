report 50198 "GST Sales Invoice Export Nepal"
{
    // --MSVRN 091117 modified--
    DefaultLayout = RDLC;
    RDLCLayout = 'ReportLayouts\GSTSalesInvoiceExportNepal.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(TransportAddress; Recvendor.Address + '  ' + Recvendor."Address 2")
            {
            }
            column(LRRRNo_SalesInvoiceHeader; "Sales Invoice Header"."LR/RR No.")
            {
            }
            column(LRRRDate_SalesInvoiceHeader; FORMAT("Sales Invoice Header"."LR/RR Date"))
            {
            }
            ...
            column(TPTMethod_SalesInvoiceHeader; "Sales Invoice Header"."Shipment Method Code")
            {
            }
            column(CountryName; RecCountryReg.Name)
            {
            }
            column(CountryNameConsignee; "Sales Invoice Header"."State name")
            {
            }
            column(LCNo_SalesInvoiceHeader; "Sales Invoice Header"."LC Number")
            {
            }
            ...
            column(Loc1; Location.Address)
            {
            }
            column(Loc2; Location."Address 2" + ', ' + Location.City)
            {
            }
            column(Loc3; State_Rec.Description + ', ' + 'Phone No.' + Location."Phone No.")
            {
            }
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem("Sales Invoice Line"; 113)
                {
                    ...
                    column(MRP; "Sales Invoice Line"."Unit Price")
                    {
                    }
                    ...
                    // column(AmtToCust; ROUND("Sales Invoice Line"."Amount To Customer", 1, '='))
                    column(AmtToCust; ROUND(Amt + SAmount + CAmount + IAmount, 1, '='))
                    {
                    }
                    ...
                    // column(EcessAmt; "Sales Invoice Line"."eCess Amount")
                    column(EcessAmt; 0)
                    {
                    }
...
DetailedGSTLedgerEntry.NEXT = 0;
                ...
                dataitem(RowIns; 2000000026)
                {
                    ...
                    begin
                        ...
                        SETRANGE(Number, 1, 15 - rowCount);//,rowstogen+3);
                    end;
                }
            ...
            begin
                ...
                IF CustRec.FINDFIRST THEN BEGIN
                    ...
                    RecCountryReg.RESET;
                    IF RecCountryReg.GET(CustRec."Country/Region Code") THEN;
...
RecState.NEXT = 0;
                ...
                IF Location2.FINDFIRST THEN BEGIN
                    RecCountryReg.RESET;
                    IF RecCountryReg.GET(Location2."Country/Region Code") THEN;
                    SaleInvHeader.RESET;
                ...
                IF SIL.FIND('-') THEN BEGIN
                    REPEAT
                        ...
                        //16225 GrandTotal += SIL."Amount To Customer";
                        GrandTotal += Amt + CAmount + SAmount + IAmount;
                        IF SIL."No." = '51157000' THEN
                            // GLRoundingOFF += SIL."Amount To Customer";
                            GLRoundingOFF += Amt + CAmount + SAmount + IAmount;
                    UNTIL SIL.NEXT = 0;
                END;
                //RKS
    ...
    var
        ...
        "--MSVRN--": Integer;
        HideCol: Boolean;
        RecCountryReg: Record "Country/Region";
