report 50012 "GST Sales Invoice Mod 2"
{
    // --MSVRN 091117 modified--
    DefaultLayout = RDLC;
    RDLCLayout = '.\ReportLayouts\GSTSalesInvoiceMod2.rdl';
        ...
        dataitem("Sales Invoice Header"; 112)
        {
            ...
            dataitem(CopyLoop; 2000000026)
            {
                ...
                dataitem("Sales Invoice Line"; 113)
                {
                    ...
                    column(TotalTDSTCSInclSHECESS_SalesInvoiceLine; TotalTcsinclShecessAmt) //16630  "Sales Invoice Line".Total TDS/TCS Incl. SHE CESS
                    {
                    }
                    ...
                    column(MRP; 0) //16630 blank "Sales Invoice Line".MRP Price
                    {
                    }
                    ...
                    column(TotalAmountSIH; ROUND(GetAmttoCustomerPostedLine("Document No.", "Line No."), 1, '=')) // 16630 "Sales Invoice Line"."Amount To Customer"
                    {
                    }
                    column(BEDAmt; 0) // 16630 "Sales Invoice Line".BED Amount
                    {
                    }
                    column(EcessAmt; 0) // 16630 "Sales Invoice Line".eCess Amount
                    {
                    }
                    column(ScessAmt; 0) // 16630 "Sales Invoice Line".Cess Amount
                    {
                    }
                    column(ExciseAmt; 0) // 16630  "Sales Invoice Line".Excise Amount
                    {
                    }
                    ...
                    begin
                        Clear(TotalTcsinclShecessAmt);
                        Clear(TCSper);
                        TCSentry.Reset();
                        TCSentry.SetRange("Document No.", "Document No.");
                        if TCSentry.FindSet() then begin
                            repeat
                                TotalTcsinclShecessAmt += TCSentry."Total TCS Including SHE CESS";
                                TCSper += TCSentry."TCS %";
                        ...
                        END;
                        TotalAmountSIH += LineAmt + IGSTAmt + CGSTAmt + SGSTAmt;
16630 */
  ...
  DetailedGSTLedgerEntry.NEXT = 0;
   16630 */
                        //rks
                        GrandTotal := GetAmttoCustomerPostedLine("Document No.", "Line No.");
                        ...
                        State1.RESET;
                        // 16630     State1.SETRANGE(State1.Code, "Sales Invoice Line".State);
                        IF State1.FIND('-') THEN
                        ...
                        IF SalesInvoiceLine1.FIND('-') THEN BEGIN
                            TotalBed := TotalBed +
                                ...
                                "Sales Invoice Header"."Posting Date", "Sales Invoice Header"."Currency Code",
                                  0, "Sales Invoice Header"."Currency Factor"), // 16630 SalesInvoiceLine1."BED Amount" 
                            ...
                            TotalEcess := TotalEcess +
                                ...
                                CurrExchRate.ExchangeAmtFCYToLCY(
                                "Sales Invoice Header"."Posting Date", "Sales Invoice Header"."Currency Code",
                                  0, "Sales Invoice Header"."Currency Factor"), // 16630  SalesInvoiceLine1."eCess Amount"
                                  Currency."Unit-Amount Rounding Precision");
                            ...
                            TotalShcess := TotalShcess +
                                ...
                                CurrExchRate.ExchangeAmtFCYToLCY(
                                "Sales Invoice Header"."Posting Date", "Sales Invoice Header"."Currency Code",
                                  0, "Sales Invoice Header"."Currency Factor"),// 16630 SalesInvoiceLine1."SHE Cess Amount" 
                                  Currency."Unit-Amount Rounding Precision");
                        ...
                        IF Quantity <> 0 THEN
                            // 16630   BuyersPrice := (Amount + "Excise Amount") / Quantity;
                             BuyersPrice := "Sales Invoice Line".Amount / "Sales Invoice Line".Quantity;
...
RecState.NEXT = 0;
                ...
                IF Location2.FINDFIRST THEN BEGIN
                    ...
                    /* 16630 IF SaleInvHeader.FINDFIRST THEN
                        HeaderTextLatest := DocumentPrint.PrintInvoiceType(SaleInvHeader, Location2);*/ // 16630
                END;
                //-----MS-PB END-----------
                Location.RESET;
                ...
                /* 16630  IF "Sales Invoice Header"."Form Code" <> '' THEN
                ...
                // 16630   FormatAddr.SalesInvShipTo(ShipToAddr, "Sales Invoice Header");
                ...
                // 16630  PostedDocDim1.SETRANGE("Table ID", DATABASE::"Sales Invoice Header");
                // 16630 PostedDocDim1.SETRANGE("Document No.", "Sales Invoice Header"."No.");
                ...
                IF Cust1.FINDFIRST THEN BEGIN
                    /* 16630    "Cust LST" := Cust1."L.S.T. No.";
                        "Cust CST" := Cust1."C.S.T. No.";
                        "Cust TIN" := Cust1."T.I.N. No."; */ // 16630
                    pin := Cust1."Pin Code";
                ...
                IF SIL.FIND('-') THEN BEGIN
                    ...
                    END;
                    TotalAmt += LineAmt + IGSTAmt + CGSTAmt + SGSTAmt;
                    GrandTotal := TotalAmt;
                    IF SIL."No." = '51157000' THEN
                        GLRoundingOFF := TotalAmt;   // 16630 */
                ...
                IF ("Sales Invoice Header"."Posting Date" >= 20190401D) THEN BEGIN
                ...
                */
                ...
                IF Cust."GST Registration No." = '' THEN BEGIN
                    ...
                    IF NOT "Sales Invoice Header"."QR Code".HASVALUE THEN BEGIN
                        ...
                        CLEAR(APIManagement);
                        PaymentText := APIManagement.CreateB2CQRPaymentURL("Sales Invoice Header", FALSE);
                        // 16630      APIManagement.CreateQRCode(PaymentText, TempBlob);
                        //MSKS
    ...
    begin
        ///  ERROR('Please contact to Mr. Laxman Singh');
    end;
    ...
    var
        TCSper: Decimal;
        TotalTcsinclShecessAmt: Decimal;
        ...
        PageTitle: Text[30];
        VAT: Code[20];
        //16630 ExcisePostingSetup: Record "Excise Posting Setup";
        "BED%": Decimal;
        "eCess%": Decimal;
        "SHECess%": Decimal;
        ...
        HideCol: Boolean;
        pin: Code[7];
        Text3707: Label 'Supply Meant For Export Under Bond Or Letter Of Undertaking Without Payment Of Integrated Tax | vide LUT Application No. ARN AD090319004201J Dt.30-03-2019''';
        Text3708: Label 'Supply Meant For Export Under Bond Or Letter Of Undertaking Without Payment Of Integrated Tax | vide LUT Application No. ARN AD240419010668Z Dt.24/04/2019''';
