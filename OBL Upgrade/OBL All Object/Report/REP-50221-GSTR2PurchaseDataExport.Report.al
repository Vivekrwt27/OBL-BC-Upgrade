report 50221 "GSTR-2 Purchase Data Export"
{
    ProcessingOnly = true;
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem("Purch. Inv. Header"; "Purch. Inv. Header")
        {
            ...
            dataitem("Purch. Inv. Line"; "Purch. Inv. Line")
            {
                ...
                begin
                    ...
                    IF "No." <> '503730' THEN BEGIN
                        ...
                        CLEAR(State2);
                        CLEAR(StateCode);
                        // IF "Purch. Inv. Header"."GST Order Address State"<> '' THEN BEGIN
                        // IF State2.GET("Purch. Inv. Header"."GST Order Address State") THEN
                        //   StateCode := State2."State Code (GST Reg. No.)";
                        // END;
                        //16225 Field N/F start
                        /* IF "Purch. Inv. Header".State <> '' THEN BEGIN
                             IF State2.GET("Purch. Inv. Header".State) THEN
                                 StateCode := State2."State Code (GST Reg. No.)"
                         END ELSE BEGIN
                             IF State2.GET("Purch. Inv. Header".State) THEN
                                 StateCode := State2."State Code (GST Reg. No.)";
                        ...
                        IF ExportB2bData AND NOT ("Purch. Inv. Header"."Vendor Posting Group" = 'FOREIGN') THEN
                            EnterB2bRowData;
                        IF (ExportImportofGoodData) AND ("Purch. Inv. Header"."Vendor Posting Group" = 'FOREIGN') AND ("Purch. Inv. Line"."GST Group Type" = "Purch. Inv. Line"."GST Group Type"::Goods) THEN
                            EnterExportRowData;
                        IF (ExportImportofServiceData) AND ("Purch. Inv. Header"."Vendor Posting Group" = 'FOREIGN') AND ("Purch. Inv. Line"."GST Group Type" = "Purch. Inv. Line"."GST Group Type"::Service) THEN
                            EnterExportSerRowData;
                    END;
                end;
            }
            ...
            begin
                ...
                END;
                IF ExportImportofGoodData THEN BEGIN
                    // TempExportExcelBuffer.CreateBookAndOpenExcel('', GSTImportDataTxt, '', COMPANYNAME, USERID);
                    TempExportExcelBuffer.CreateNewBook(GSTImportDataTxt);//16767
                    TempExportExcelBuffer.WriteSheet(GSTImportDataTxt, CompanyName, UserId);//16767
                    // TempExportExcelBuffer.WriteSheet(GSTExportDataTxt,COMPANYNAME,USERID);
                    TempExportExcelBuffer.CloseBook;
                    TempExportExcelBuffer.SetFriendlyFilename(StrSubstNo(GSTImportDataTxt, CurrentDateTime, UserId));//16767
                    TempExportExcelBuffer.OpenExcel();
                    // TempExportExcelBuffer.OpenExcel;
                    ///    TempExportExcelBuffer.GiveUserControl;
                END;
                IF ExportImportofServiceData THEN BEGIN
                    // TempExportExcelBuffer.CreateBookAndOpenExcel('', GSTImportDataTxt, '', COMPANYNAME, USERID);
                    TempExportExcelBuffer.CreateNewBook(GSTImportDataTxt);//16767
                    TempExportExcelBuffer.WriteSheet(GSTImportDataTxt, CompanyName, UserId);//16767
                    // TempExportExcelBuffer.WriteSheet(GSTExportDataTxt,COMPANYNAME,USERID);
                    TempExportExcelBuffer.CloseBook;
                    TempExportExcelBuffer.SetFriendlyFilename(StrSubstNo(GSTImportDataTxt, CurrentDateTime, UserId));//16767
                    TempExportExcelBuffer.OpenExcel();//16767
                    // TempExportExcelBuffer.OpenExcel;
                    ///    TempExportExcelBuffer.GiveUserControl;
                END;
            end;
            ...
            begin
                ...
                IF NOT (ExportB2bData OR ExportCrNoteData OR ExportImportofGoodData OR ExportImportofServiceData OR ExportJnlVoucherData) THEN
                    CurrReport.BREAK;
                ...
                IF ExportImportofGoodData THEN
                    EnterCell(RowNo, ColumnNo, GSTImportDataTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
                IF ExportImportofServiceData THEN
                    EnterCell(RowNo, ColumnNo, GSTImportDataTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
                IF ExportJnlVoucherData THEN
        ...
        dataitem("Purch. Cr. Memo Hdr."; "Purch. Cr. Memo Hdr.")
        {
            dataitem("Purch. Cr. Memo Line"; "Purch. Cr. Memo Line")
            {
                ...
                trigger OnAfterGetRecord()
                begin
                    IF "Purch. Cr. Memo Line"."No." <> '503730' THEN BEGIN
                        IF Location.GET("Purch. Cr. Memo Line"."Location Code") THEN;
                        CLEAR(State2);
                        CLEAR(StateCode2);
                        // IF "Purch. Cr. Memo Hdr."."GST Order Address State" <> '' THEN BEGIN
                        // IF State2.GET("Purch. Cr. Memo Hdr."."GST Order Address State") THEN
                        //   StateCode2 := State2."State Code (GST Reg. No.)"
                        // END;
                        //16225 Field N/F start
                        /* IF "Purch. Inv. Header".State <> '' THEN BEGIN
                             IF State2.GET("Purch. Cr. Memo Hdr.".State) THEN
                                 StateCode2 := State2."State Code (GST Reg. No.)"
                         END ELSE BEGIN
                             IF State2.GET("Purch. Cr. Memo Hdr.".State) THEN
                                 StateCode2 := State2."State Code (GST Reg. No.)"
                         END;*///16225 Field N/F End
                        Customer.GET("Purch. Cr. Memo Line"."Buy-from Vendor No.");
                        // Customer.GET("Bill-to Customer No.");
                        EnterCrNoteRowData;
                    END;
                end;
        ...
        dataitem("Detailed GST Ledger Entry"; "Detailed GST Ledger Entry")
        {
            ...
            var
                ...
                QueryForGSTAmount: Query "Dtld. GST Ledg. Entry Comp";
                detailGstLedgerEntryInfo: Record "Detailed GST Ledger Entry Info";
                DetGstLedgerEntryInfo: Record "Detailed GST Ledger Entry Info";
            begin
                detailGstLedgerEntryInfo.Get("Detailed GST Ledger Entry"."Entry No.");
                ColumnNo := 1;
                IF ExportJnlVoucherData THEN BEGIN
                    ...
                    IF (CLE.Reversed = FALSE) AND (CLE."Document Type" <> CLE."Document Type"::Refund) THEN BEGIN
                        ...
                        DCLE.RESET;
                        DCLE.SETRANGE(DCLE."Cust. Ledger Entry No.", detailGstLedgerEntryInfo."CLE/VLE Entry No.");
        ...
        dataitem("Detailed GST Ledger Entry1"; "Detailed GST Ledger Entry")
        {
            ...
            var
                DetailGstledgerEntryInfo: Record "Detailed GST Ledger Entry Info";
            begin
                DetailGstledgerEntryInfo.Get("Detailed GST Ledger Entry1"."Entry No.");
                ...
                //  DetailedGSTLedgerEntry3.SETRANGE("Document No.","Original Doc. No.");
                DetailedGSTLedgerEntry3.SETRANGE("Document No.", DetailGstledgerEntryInfo."Original Doc. No.");
                ...
                IF DocNo <> "Document No." THEN BEGIN
                    ...
                    IF ExportJnlVoucherData THEN BEGIN
                        ...
                        ColumnNo := 1;
                        CLE.RESET;
                        CLE.SETRANGE("Entry No.", DetailGstledgerEntryInfo."CLE/VLE Entry No.");
                        // CLE.SETRANGE("Entry No.","CLE/VLE Entry No.");
                        IF CLE.FINDFIRST THEN;
                        ...
                        IF DetailGstledgerEntryInfo."Buyer/Seller State Code" <> '' THEN
                            IF State2.GET(DetailGstledgerEntryInfo."Buyer/Seller State Code") THEN
                                StateCode := State2."State Code (GST Reg. No.)";
                        //State Code
                ...
                DocNo := "Document No.";
                OriginalDocNo := DetailGstledgerEntryInfo."Original Doc. No.";
        ...
        dataitem("Detailed GST Ledger Entry2"; "Detailed GST Ledger Entry")
        {
            ...
            trigger OnAfterGetRecord()
            var
                DetailGstLadgEntInfo: Record "Detailed GST Ledger Entry Info";
            begin
                DetailGstLadgEntInfo.Get("Entry No.");
                    ...
                    IF Cnt > 0 THEN
                        IF "Document Type" = "Document Type"::Invoice THEN BEGIN
                            IF ExportJnlVoucherData THEN BEGIN
                                ...
                                RowNo += 1;
                                CLE.RESET;
                                CLE.SETRANGE("Entry No.", DetailGstLadgEntInfo."CLE/VLE Entry No.");
                                //16225  CLE.SETRANGE("Entry No.","CLE/VLE Entry No.");DetailGstLadgEntInfo
                                IF CLE.FINDFIRST THEN;
                                ...
                                CLEAR(State2);
                                CLEAR(StateCode);
                                IF DetailGstLadgEntInfo."Buyer/Seller State Code" <> '' THEN
                                    //16225 IF "Buyer/Seller State Code" <> '' THEN
                                    IF State2.GET(DetailGstLadgEntInfo."Buyer/Seller State Code") THEN
                                        StateCode := State2."State Code (GST Reg. No.)";
                                //State Code
                ...
                DocNo := "Document No.";
                //16225   OriginalDocNo := "Original Doc. No.";
                OriginalDocNo := DetailGstLadgEntInfo."Original Doc. No.";
            end;
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                ...
                field(ExportImportofGoodData; ExportImportofGoodData)
                {
                    Caption = 'Export Import of Goods';
                    ApplicationArea = All;
                }
                field(ExportImportofServiceData; ExportImportofServiceData)
                {
                    Caption = 'Export Import of Services';
                    ApplicationArea = All;
                }
    ...
    begin
        // TempExportExcelBuffer.DELETEALL;
        // TempB2bExcelBuffer.DELETEALL;
        // TempJnlVoucherExcelBuffer.DELETEALL;
        // TempCrNoteExcelBuffer.DELETEALL;
        //
    end;
    ...
    var
        ...
        ExportImportofGoodData: Boolean;
        ExportImportofServiceData: Boolean;
        ...
        SupplierTypeTxt: Label 'Supplier Type';
        ...
        ImportTypeTxt: Label 'Import Type';
        ...
        SupplierGSTINTxt: Label 'Supplier GSTN';
        SupplierNameTxt: Label 'Supplier Name';
        ITCEligibilityTxt: Label 'ITC Eligibility';
        IGSTITCTxt: Label 'IGST ITC';
        CGSTITCTxt: Label 'CGST ITC';
        SGSTITCTxt: Label 'SGST ITC';
        CESSITCTxt: Label 'CESS ITC';
        BillofEntryStatusTxt: Label 'Bill of Entry Status';
        BillofEntryNumberTxt: Label 'Bill of Entry Number';
        BillofEntryDateTxt: Label 'Bill of Entry Date';
        OriginalBillofEntryNumberTxt: Label 'Original Bill of Entry Number';
        OriginalBillofEntryDateTxt: Label 'Original Bill of Entry Date';
        BillofEntryValueTxt: Label 'Bill of Entry Value';
        AssessableValueTxt: Label 'Assessable Value';
        ISSEZTxt: Label 'IS SEZ';
        SEZGSTINTxt: Label 'SEZ GSTIN';
    ...
    begin
        EnterCell(RowNo, ColumnNo, BuyerGSTINTxt, TRUE, FALSE, TRUE, TRUE, '', TempB2bExcelBuffer."Cell Type"::Text, TempB2bExcelBuffer);
        ColumnNo += 1;
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, SupplierNameTxt, TRUE, FALSE, TRUE, TRUE, '', TempB2bExcelBuffer."Cell Type"::Text, TempB2bExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, PlaceofSupplyTxt, TRUE, FALSE, TRUE, TRUE, '', TempB2bExcelBuffer."Cell Type"::Text, TempB2bExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, ITCEligibilityTxt, TRUE, FALSE, TRUE, TRUE, '', TempB2bExcelBuffer."Cell Type"::Text, TempB2bExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, IGSTITCTxt, TRUE, FALSE, TRUE, TRUE, '', TempB2bExcelBuffer."Cell Type"::Text, TempB2bExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, CGSTITCTxt, TRUE, FALSE, TRUE, TRUE, '', TempB2bExcelBuffer."Cell Type"::Text, TempB2bExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, SGSTITCTxt, TRUE, FALSE, TRUE, TRUE, '', TempB2bExcelBuffer."Cell Type"::Text, TempB2bExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, CESSITCTxt, TRUE, FALSE, TRUE, TRUE, '', TempB2bExcelBuffer."Cell Type"::Text, TempB2bExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, ReverseChargeTxt, TRUE, FALSE, TRUE, TRUE, '', TempB2bExcelBuffer."Cell Type"::Text, TempB2bExcelBuffer);
        ColumnNo += 1;
    ...
    begin
        EnterCell(RowNo, ColumnNo, BuyerGSTINTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, BillofEntryStatusTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, ImportTypeTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, 'Port Code', TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, BillofEntryNumberTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, BillofEntryDateTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, OriginalBillofEntryNumberTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, OriginalBillofEntryDateTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, BillofEntryValueTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, AssessableValueTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, ITCEligibilityTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, IGSTITCTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, CESSITCTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, ISSEZTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, SEZGSTINTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, 'CESS', TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
    ...
    begin
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, SupplierTypeTxt, TRUE, FALSE, TRUE, TRUE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, SupplierNameTxt, TRUE, FALSE, TRUE, TRUE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, ITCEligibilityTxt, TRUE, FALSE, TRUE, TRUE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, IGSTITCTxt, TRUE, FALSE, TRUE, TRUE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, CGSTITCTxt, TRUE, FALSE, TRUE, TRUE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, SGSTITCTxt, TRUE, FALSE, TRUE, TRUE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, CESSITCTxt, TRUE, FALSE, TRUE, TRUE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        CrNoteRowNo := RowNo;
    end;

    local procedure PrePareVoucherHeading()
    begin
        EnterCell(RowNo, ColumnNo, BuyerGSTINTxt, TRUE, FALSE, TRUE, TRUE, '', TempJnlVoucherExcelBuffer."Cell Type"::Text, TempJnlVoucherExcelBuffer);
        ColumnNo += 1;
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, 'Gross Advance Paid (Excluding tax)', TRUE, FALSE, TRUE, TRUE, '', TempJnlVoucherExcelBuffer."Cell Type"::Text, TempJnlVoucherExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, 'Gross Advance Adjusted(excluding tax)', TRUE, FALSE, TRUE, TRUE, '', TempJnlVoucherExcelBuffer."Cell Type"::Text, TempJnlVoucherExcelBuffer);
        ColumnNo += 1;
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, SupplierGSTINTxt, TRUE, FALSE, TRUE, TRUE, '', TempJnlVoucherExcelBuffer."Cell Type"::Text, TempJnlVoucherExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, SupplierNameTxt, TRUE, FALSE, TRUE, TRUE, '', TempJnlVoucherExcelBuffer."Cell Type"::Text, TempJnlVoucherExcelBuffer);
        VoucherRowNo := RowNo;
    end;

    local procedure PrePareExportSerHeading()
    begin
        EnterCell(RowNo, ColumnNo, BuyerGSTINTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, SupplierNameTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, PlaceofSupplyTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, ITCEligibilityTxt, TRUE, FALSE, TRUE, TRUE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
    ...
    begin
        ...
        DtldGSTLedgEntryComp.SETFILTER(Source_Type, 'Vendor');
    ...
    begin
        ...
        CustLedgEntryGSTNo.SETRANGE("Document No.", "Purch. Inv. Header"."No.");
        ...
        RecDGLE.RESET();
        RecDGLE.SETRANGE("Document No.", "Purch. Inv. Header"."No.");
        ...
        WITH "Purch. Inv. Line" DO BEGIN
            TotalAmount := "Purch. Inv. Line"."Line Amount" + CGSTAmt + SGSTAmt + IGSTAmt;//16225
            IF B2bRowNo = 0 THEN
                B2bRowNo := RowNo;
            B2bRowNo += 1;
            RowNo := B2bRowNo;
            ...
            ColumnNo += 1;
            EnterCell(RowNo, ColumnNo, FORMAT("Purch. Inv. Header"."Vendor Invoice No."), FALSE, FALSE, FALSE, FALSE, '', TempB2bExcelBuffer."Cell Type"::Text, TempB2bExcelBuffer);
            ColumnNo += 1;
            EnterCell(RowNo, ColumnNo, FORMAT(''), FALSE, FALSE, FALSE, FALSE, '', TempB2bExcelBuffer."Cell Type"::Text, TempB2bExcelBuffer);
            ColumnNo += 1;
            //16225  EnterCell(RowNo, ColumnNo, FORMAT(ROUND("Amount To Vendor", 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempB2bExcelBuffer."Cell Type"::Number, TempB2bExcelBuffer);
            ...
            ColumnNo += 1;
            //  EnterCell(RowNo,ColumnNo,FORMAT("Purch. Inv. Header"."Vendor GST Reg. No."),FALSE,FALSE,FALSE,FALSE,'',TempB2bExcelBuffer."Cell Type"::Number,TempB2bExcelBuffer);
            ColumnNo += 1;
            EnterCell(RowNo, ColumnNo, FORMAT("Purch. Inv. Header"."Buy-from Vendor Name"), FALSE, FALSE, FALSE, FALSE, '', TempB2bExcelBuffer."Cell Type"::Number, TempB2bExcelBuffer);
            ColumnNo += 1;
            EnterCell(RowNo, ColumnNo, FORMAT(StateCode), FALSE, FALSE, FALSE, FALSE, '', TempB2bExcelBuffer."Cell Type"::Number, TempB2bExcelBuffer);
            ColumnNo += 1;
            EnterCell(RowNo, ColumnNo, FORMAT('IP'), FALSE, FALSE, FALSE, FALSE, '', TempB2bExcelBuffer."Cell Type"::Number, TempB2bExcelBuffer);
            ColumnNo += 1;
            EnterCell(RowNo, ColumnNo, FORMAT(ROUND(ABS(IGSTAmount), 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempB2bExcelBuffer."Cell Type"::Number, TempB2bExcelBuffer);
            ColumnNo += 1;
            EnterCell(RowNo, ColumnNo, FORMAT(ROUND(ABS(CGSTAmount), 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempB2bExcelBuffer."Cell Type"::Number, TempB2bExcelBuffer);
            ...
            //  IF DetailedGSTLedgerEntry."e-Comm. Merchant Id" <> '' THEN
            //    EnterCell(RowNo,ColumnNo,FORMAT('Y'),FALSE,FALSE,FALSE,FALSE,'',TempB2bExcelBuffer."Cell Type"::Text,TempB2bExcelBuffer)
            //  ELSE
            //    EnterCell(RowNo,ColumnNo,FORMAT('N'),FALSE,FALSE,FALSE,FALSE,'',TempB2bExcelBuffer."Cell Type"::Text,TempB2bExcelBuffer);
            //  ColumnNo +=1;
            //  EnterCell(RowNo,ColumnNo,FORMAT(DetailedGSTLedgerEntry."e-Comm. Operator GST Reg. No."),FALSE,FALSE,FALSE,FALSE,'',TempB2bExcelBuffer."Cell Type"::Text,TempB2bExcelBuffer);
            //  ColumnNo +=1;
            //  //ALLE_ABHISHEK 28/09/18
            ...
            //  //ALLE_ABHISHEK 28/09/18
            //  ColumnNo +=1;
            //  EnterCell(RowNo,ColumnNo,FORMAT("Purch. Inv. Header"."Ship-to Name"),FALSE,FALSE,FALSE,FALSE,'',TempB2bExcelBuffer."Cell Type"::Text,TempB2bExcelBuffer);
            //  ColumnNo +=1;
    ...
    begin
        ...
        RecDGLE.SETRANGE("Transaction Type", RecDGLE."Transaction Type"::Purchase);
        RecDGLE.SETRANGE(RecDGLE."Document No.", "Purch. Inv. Header"."No.");
        IF RecDGLE.FINDFIRST THEN;
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT('Outside India'), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Inv. Header"."Location Code"), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Inv. Header"."Bill of Entry No."), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Inv. Header"."Bill of Entry Date"), FALSE, FALSE, FALSE, FALSE, 'dd/mm/yyyy', TempExportExcelBuffer."Cell Type"::Date, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Inv. Header"."No."), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Inv. Header"."Posting Date"), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Inv. Header"."Bill of Entry Value"), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT(RecDGLE."GST Assessable Value"), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Inv. Line"."HSN/SAC Code"), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Inv. Line".Description), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT(ROUND("Purch. Inv. Line".Quantity, 0.001, '=')), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Number, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT(''), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT(ROUND(RecDGLE."GST Base Amount", 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Number, TempExportExcelBuffer);
        ColumnNo += 1;
        ...
        EnterCell(RowNo, ColumnNo, FORMAT(ROUND(RecDGLE."GST %", 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Number, TempExportExcelBuffer);
        ColumnNo += 1;
        GetGSTCompAmount("Purch. Inv. Line"."Document No.", "Purch. Inv. Line"."Line No.", GlobalDocumentType::Invoice, GlobalTransactionType::Purchase, CGSTAmount, SGSTAmount, IGSTAmount, ECessAmount);
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT(RecDGLE."Eligibility for ITC"), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Number, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT(ROUND(ABS(IGSTAmount), 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Number, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT(''), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Number, TempExportExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT('Y'), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Number, TempExportExcelBuffer);
        ColumnNo += 1;
        //ALLE_ABHISHEK 28/09/18
    ...
    begin
        ...
        CustLedgEntryGSTNo.SETRANGE("Document No.", "Purch. Cr. Memo Hdr."."No.");
        IF NOT CustLedgEntryGSTNo.FINDFIRST THEN
            CustLedgEntryGSTNo.INIT;
        //ALLE_ABHISHEK 28/09/18
        ...
        ColumnNo := 1;
        TotalAmount += "Purch. Cr. Memo Line"."Line Amount" + CGSTAmt + SGSTAmt + IGSTAmt;
        ...
        */
        ...
        ColumnNo += 1;
        IF FORMAT("Purch. Cr. Memo Line"."Posting Date") > '01/07/17' THEN
        ...
        IF "Purch. Cr. Memo Hdr."."Applies-to Doc. No." <> '' THEN
            EnterCell(RowNo, ColumnNo, FORMAT("Purch. Cr. Memo Hdr."."Applies-to Doc. No."), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer)
        ELSE
            //Alle EnterCell(RowNo,ColumnNo,FORMAT("Purch. Cr. Memo Hdr."."Applied Invoice"),FALSE,FALSE,FALSE,FALSE,'',TempCrNoteExcelBuffer."Cell Type"::Text,TempCrNoteExcelBuffer);
            CustLedgEntry.RESET;
        CustLedgEntry.SETRANGE("Document Type", CustLedgEntry."Document Type"::Invoice);
        //Alle  CustLedgEntry.SETRANGE("Document No.","Purch. Cr. Memo Hdr."."Applied Invoice");
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Cr. Memo Line"."Document No."), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Cr. Memo Line"."Posting Date"), FALSE, FALSE, FALSE, FALSE, 'dd/mm/yyyy', TempCrNoteExcelBuffer."Cell Type"::Date, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT('Normal'), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        ...
        EnterCell(RowNo, ColumnNo, FORMAT(''), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        //16225 EnterCell(RowNo, ColumnNo, FORMAT(ROUND("Amount To Vendor", 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Number, TempCrNoteExcelBuffer);
        ...
        ColumnNo += 1;
        // IF "Purch. Cr. Memo Hdr."."GST Customer Type" = "Purch. Cr. Memo Hdr."."GST Customer Type" :: Registered THEN
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Cr. Memo Line"."HSN/SAC Code"), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Cr. Memo Line".Description), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT(ROUND("Purch. Cr. Memo Line".Quantity, 0.001, '=')), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Number, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        ...
        ColumnNo += 1;
        GetGSTCompAmount("Purch. Cr. Memo Line"."Document No.", "Purch. Cr. Memo Line"."Line No.", GlobalDocumentType::"Credit Memo", GlobalTransactionType::Sales, CGSTAmount, SGSTAmount, IGSTAmount, ECessAmount);
        ...
        //  EnterCell(RowNo,ColumnNo,FORMAT("Purch. Cr. Memo Hdr."."Vendor GST Reg. No."),FALSE,FALSE,FALSE,FALSE,'',TempCrNoteExcelBuffer."Cell Type"::Text,TempCrNoteExcelBuffer);
        //ALLE_ABHISHEK 28/09/18
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT("Purch. Cr. Memo Hdr."."Buy-from Vendor Name"), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        ...
        ELSE
            //Alle  IF (("Purch. Cr. Memo Hdr."."GST Bill-to State Code") <> ("Purch. Cr. Memo Hdr."."Location State Code")) AND ("Amount To Customer" > 250000) THEN
            EnterCell(RowNo, ColumnNo, FORMAT('B2CL'), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Text, TempCrNoteExcelBuffer);
        //  ELSE
        //16225  IF ("Amount To Vendor" < 250000) THEN
        ...
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT('IP'), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Number, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT(ROUND(ABS(IGSTAmount), 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Number, TempCrNoteExcelBuffer);
        ColumnNo += 1;
        EnterCell(RowNo, ColumnNo, FORMAT(ROUND(ABS(CGSTAmount), 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempCrNoteExcelBuffer."Cell Type"::Number, TempCrNoteExcelBuffer);
        ...
        */
        ...
        IF DetailedGSTLedgerEntry3.FINDSET THEN
            REPEAT
                ...
                EnterCell(RowNo, ColumnNo, FORMAT('Add'), FALSE, FALSE, FALSE, FALSE, '', TempJnlVoucherExcelBuffer."Cell Type"::Text, TempJnlVoucherExcelBuffer);
                ColumnNo += 1;
                IF DetailedGSTLedgerEntry3."Document Type" = DetailedGSTLedgerEntry3."Document Type"::"Credit Memo" THEN
                    EnterCell(RowNo, ColumnNo, FORMAT('Payment Voucher'), FALSE, FALSE, FALSE, FALSE, '', TempJnlVoucherExcelBuffer."Cell Type"::Text, TempJnlVoucherExcelBuffer)
                ELSE
                    EnterCell(RowNo, ColumnNo, FORMAT('Payment Voucher'), FALSE, FALSE, FALSE, FALSE, '', TempJnlVoucherExcelBuffer."Cell Type"::Text, TempJnlVoucherExcelBuffer);
                ...
                ColumnNo += 1;
                GetGSTCompAmount(DetailedGSTLedgerEntry3."Document No.", DetailedGSTLedgerEntry3."Document Line No.", DetailedGSTLedgerEntry3."Document Type".AsInteger(), GlobalTransactionType::Sales, CGSTAmount, SGSTAmount, IGSTAmount, ECessAmount);
                EnterCell(RowNo, ColumnNo, FORMAT(ROUND(ABS(IGSTAmount), 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempJnlVoucherExcelBuffer."Cell Type"::Number, TempJnlVoucherExcelBuffer);
    ...
    local procedure EnterExportSerRowData()
    var
        RecDGLE: Record "Detailed GST Ledger Entry";
    begin
        //ALLE_ABHISHEK 28/09/18
        ...
        WITH "Purch. Inv. Line" DO BEGIN
            ...
            EnterCell(RowNo, ColumnNo, FORMAT(''), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
            ColumnNo += 1;
            //16225 EnterCell(RowNo, ColumnNo, FORMAT("Amount To Vendor"), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
            EnterCell(RowNo, ColumnNo, FORMAT(RecDGLE."GST Base Amount" + RecDGLE."GST Amount"), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer);
            ColumnNo += 1;
            ...
            EnterCell(RowNo, ColumnNo, FORMAT(ROUND(RecDGLE."GST %", 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Number, TempExportExcelBuffer);
            ColumnNo += 1;
            GetGSTCompAmount("Document No.", "Line No.", GlobalDocumentType::Invoice, GlobalTransactionType::Purchase, CGSTAmount, SGSTAmount, IGSTAmount, ECessAmount);
            EnterCell(RowNo, ColumnNo, FORMAT(ROUND(ABS(IGSTAmount), 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Number, TempExportExcelBuffer);
            ColumnNo += 1;
            EnterCell(RowNo, ColumnNo, FORMAT(ROUND(ABS(ECessAmount), 0.01, '=')), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Number, TempExportExcelBuffer);
            ColumnNo += 1;
            EnterCell(RowNo, ColumnNo, FORMAT("Purch. Inv. Header"."Buy-from Vendor Name"), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Number, TempExportExcelBuffer);
            ColumnNo += 1;
            IF StateCode <> '' THEN
                EnterCell(RowNo, ColumnNo, FORMAT(StateCode), FALSE, FALSE, FALSE, FALSE, '', TempExportExcelBuffer."Cell Type"::Text, TempExportExcelBuffer)
            ELSE
