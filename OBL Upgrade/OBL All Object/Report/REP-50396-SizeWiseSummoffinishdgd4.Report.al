report 50396 "Size-Wise Summ. of finishd gd4"
{
    // 1. TRI NP 100510-Grouping done by Production Plant Code
    DefaultLayout = RDLC;
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = all;
    RDLCLayout = '.\ReportLayouts\SizeWiseSummoffinishdgd4.rdl';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            dataitem(Integer; Integer)
            {
                ...
                column(GlobalSizeCode; GlobalSizeCode)
                {
                }
                column(GlobalPackingCode; GlobalPackingCode)
                {
                }
                column(GlobalPlantCode; GlobalPlantCode)
                {
                }
                ...
                begin
                    IntegerArrayInt += 1;
                    //IF IntegerArrayInt > 1 THEN BEGIN
                    OutputQuantityCR := 0;
                    OutputQuantitySQ := 0;
                    OpQtyCrtn := 0;
                    OpQtySqm := 0;
                    ...
                    ILE.SETRANGE("Item No.", Item."No."); //Added Later 6700
                    ILE.SETRANGE(ILE."Production Plant Code", PlantCodetoFilter[IntegerArrayInt]);
                    ...
                    //      MESSAGE('%1   %2   %3   %4',OutputQtyCR,OutputQtySQ,RecptQtymt,PlantCodetoFilter[IntegerArrayInt]);
                    IF tgWarehouse THEN BEGIN
                        ILE.RESET;
                        ILE.SETCURRENTKEY("Production Plant Code", "Size Code", "Packing Code", "Posting Date");
                        ILE.SETRANGE("Item No.", Item."No."); //Added Later 6700
                        ...
                        IF RecItem.GET(ILE."Item No.") THEN
                            SaleIssueGW += ROUND(Item."Gross Weight" * ILE.Quantity, 0.001, '=') / 1000;
                    ...
                    IF PlantCodetoFilter[IntegerArrayInt] <> '' THEN
                        GlobalPlantCode := PlantCodetoFilter[IntegerArrayInt];
                end;

                trigger OnPreDataItem()
                begin
                    //IF ARRAYLEN(PlantCodetoFilter[ArrayInt]) > 1 THEN
                    //COMPRESSARRAY(PlantCodetoFilter);
                    IF ArrayInt <> 0 THEN
                        SETRANGE(Number, 1, ArrayInt)
                    ELSE
                        SETRANGE(Number, 1, 1);
                    //ELSE
                    //EXIT;
                    //MESSAGE(FORMAT(ARRAYLEN(PlantCodetoFilter)));
                    CLEAR(IntegerArrayInt);
                end;
            }
            ...
            begin
                ...
                IF ILE.FINDFIRST THEN
                    REPEAT
                        IF LastProdPlantCode <> ILE."Production Plant Code" THEN BEGIN
                            //MESSAGE(ILE."Production Plant Code");
                            ArrayInt += 1;
                            PlantCodetoFilter[ArrayInt] := ILE."Production Plant Code";
                        END;
                        LastProdPlantCode := ILE."Production Plant Code";
                    UNTIL
                    ILE.NEXT = 0;
             ...
             */
                ...
                END;
                  */
                GlobalSizeCode := Item."Size Code";
                GlobalPackingCode := Item."Packing Code";
                GlobalPlantCode := Item."Default Prod. Plant Code";
                //MESSAGE('%1  %2  %3',GlobalPlantCode,GlobalSizeCode,ArrayInt);
    ...
    END;
    */
        //16225 Funcaion N/F  RepAuditMgt.CreateAudit(50140)
    ...
    var
        ...
        Int: Integer;
        PlantCodetoFilter: array[30] of Code[30];
        ArrayInt: Integer;
        IntegerArrayInt: Integer;
