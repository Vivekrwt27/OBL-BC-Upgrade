report 50193 "Inventory Valuation Report Lo2"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'ReportLayouts\InventoryValuationReportLo2.rdl';
    ...
    dataset
    {
        dataitem(Item; 27)
        {
            RequestFilterFields = "No.", "Inventory Posting Group", "Location Filter", "Statistics Group";
            ...
            dataitem(Location; 14)
            {
                ...
                column(InvoicedQuantity; InvQty)
                {
                }
                column(CostAmt; CAmt)
                {
                    ...
                    WITH "Value Entry" DO BEGIN
                        ...
                        IF FINDFIRST THEN BEGIN
                            REPEAT
                                ...
                                IF "Item Ledger Entry Quantity" <> 0 THEN BEGIN
                                    ...
                                    ELSE BEGIN
                                        ...
                                        ELSE BEGIN
                                            ...
                                            IF "Item Ledger Entry Type" = "Item Ledger Entry Type"::Sale THEN
                                                ShipDecreaseDetail[1] += -"Item Ledger Entry Quantity"
                                            ELSE
                                                IF ("Item Ledger Entry Type" = "Item Ledger Entry Type"::"Negative Adjmt.") AND ("Direct Consumption Entries" = FALSE) THEN
                                                    ShipDecreaseDetail[2] += -"Item Ledger Entry Quantity"
                                                ELSE
                                                    IF ("Item Ledger Entry Type" = "Item Ledger Entry Type"::"Negative Adjmt.") AND ("Direct Consumption Entries" = TRUE) THEN
                                                        ShipDecreaseDetail[3] += -"Item Ledger Entry Quantity"
                                                    ELSE
                                                        IF "Item Ledger Entry Type" = "Item Ledger Entry Type"::Consumption THEN
                                                            ShipDecreaseDetail[4] += -"Item Ledger Entry Quantity"
                                                        ELSE
                                                            IF ("Item Ledger Entry Type" = "Item Ledger Entry Type"::Transfer) THEN
                                                                ShipDecreaseDetail[5] += -"Item Ledger Entry Quantity"
                                                            ELSE
                                                                IF ("Item Ledger Entry Type" = "Item Ledger Entry Type"::Purchase) THEN
                                                                    ShipDecreaseDetail[6] += -"Item Ledger Entry Quantity"
                                                                ELSE
                                                                    IF ("Item Ledger Entry Type" = "Item Ledger Entry Type"::Output) THEN
                                                                        ShipDecreaseDetail[7] += -"Item Ledger Entry Quantity"
                                ...
                                CAmt += "Value Entry"."Cost Amount (Actual)"
                            UNTIL
                            NEXT = 0;
                        END ELSE
                            CurrReport.SKIP;
                ...
                begin
                    ...
                    IF (Item.GETFILTER("Location Filter") = 'SKD-STORE') OR (Item.GETFILTER("Location Filter") = 'DRA-STORE')
                        OR (Item.GETFILTER("Location Filter") = 'HSK-STORE') THEN
                        SETFILTER(Code, GetLocations(Item.GETFILTER("Location Filter")))
                    ELSE
                        //MSAK.END 010515
                        SETFILTER(Code, Item.GETFILTER("Location Filter"));
    ...
    begin
        CompInfo.GET();
        "Value Entry".SETCURRENTKEY("Item No.", "Posting Date", "Item Ledger Entry Type", "Entry Type", "Variance Type", "Item Charge No.", "Location Code", "Variant Code", "Global Dimension 1 Code", "Global Dimension 2 Code", "Source Type", "Source No.");
    end;
