report 50239 "Closing Stock - Store Associat"
{
    DefaultLayout = RDLC;
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = all;
    RDLCLayout = '.\ReportLayouts\ClosingStockStoreAssociat.rdl';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            column(SalesOrderQty; SalesOrderQty)
            {
            }
            ...
            begin
                ...
                SalesOrderQty := CalculateSOinOrients(Item."No.");
                ...
                //CurrReport.SKIP;
                IF ("Net Change" = 0) AND (SalesOrderQty = 0) THEN
                    CurrReport.SKIP;
                QtyInSqm := "Net Change";
                ...
                /*
                  ...
                  END
                   ELSE IF ((Item."Net Change" <> 0) AND (Miniqty = FALSE)) THEN BEGIN
                    ItemCostMgt.CalculateAverageCost(Item,AverageCostLCY,AverageCostACY);
                    value := Item."Net Change" * AverageCostLCY;
                    //Quantity := Item."Net Change";
                //END;
    ...
    procedure CalculateSOinOrients(ItemCode: Code[20]) SOQty: Decimal
    var
        SalesHeader: Record "Sales Header";
        SalesLine: Record "Sales Line";
    begin
        SalesLine.RESET;
        SalesLine.CHANGECOMPANY('Orient Bell Limited');
        SalesLine.SETCURRENTKEY("Document Type", "Variant Code", "Location Code", "No.", Type, Status);
        SalesLine.SETFILTER("Document Type", '%1', SalesLine."Document Type"::Order);
        SalesLine.SETFILTER("Location Code", '%1', 'DP-MORBI');
        SalesLine.SETFILTER("No.", '%1', ItemCode);
        ...
        IF SalesLine.FINDFIRST THEN BEGIN
            REPEAT
                ...
                SalesHeader.RESET;
                SalesHeader.CHANGECOMPANY('Orient Bell Limited');
                SalesHeader.SETRANGE("Document Type", SalesLine."Document Type");
                SalesHeader.SETFILTER("No.", '%1', SalesLine."Document No.");
                IF SalesHeader.FINDFIRST THEN BEGIN
                    IF SalesHeader.Status = SalesHeader.Status::"Price Approved" THEN
                        IF SalesHeader."Payment Date 3" <> 0D THEN
                            SOQty += SalesLine."Outstanding Quantity";
                END;
