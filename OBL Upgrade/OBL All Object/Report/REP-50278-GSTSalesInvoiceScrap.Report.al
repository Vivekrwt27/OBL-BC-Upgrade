report 50278 "GST Sales Invoice -Scrap"
{
    DefaultLayout = RDLC;
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = all;
    RDLCLayout = '.\ReportLayouts\GSTSalesInvoiceScrap.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(ship_add1; "Sales Invoice Header"."Ship-to Name 2")
            {
            }
            column(E_way; "Sales Invoice Header"."E-Way Bill No.")
            {
            }
            column(ship_add2; "Sales Invoice Header"."Ship-to Address 2")
            {
            }
            column(ship_city_post; ShipToAdd[4] + '- ' + ShipToAdd[5])
            {
            }
            column(ship_state; shiptosta)
            {
            }
            ...
            column(NetInWord; UpperCase(NetInWord[1]))
            {
            }
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem("Sales Invoice Line"; "Sales Invoice Line")
                {
                    ...
                    column(TDSTCS_SalesInvoiceLine; TDSTCSPer)
                    {
                    }
                    //16225  column(TotalTDSTCSInclSHECESS_SalesInvoiceLine; "Sales Invoice Line"."Total TDS/TCS Incl. SHE CESS")
                    column(TotalTDSTCSInclSHECESS_SalesInvoiceLine; TotalTDSTCSInclSHECESS)
                    {
                    }
                    ...
                    column(Description2; Description + ' ' + "Description 2" + ' ' + Remarks)
                    {
                    }
                    ...
                    column(QtySqMtr; Quantity)
                    {
                    }
                    //16225    column(MRP; "Sales Invoice Line"."MRP Price")
                    ...
                    column(BuyersRatePerSqMtr; "Sales Invoice Line"."Unit Price")
                    {
                    }
                    column(Value; Value)
                    {
                    ...
                    column(Disamt; ABS("Sales Invoice Header"."Discount Amount"))
                    {
                    }
                    ...
                    column(EcessAmt; eCessAmount)
                    {
                    }
                    //16225    column(ScessAmt; "Sales Invoice Line". SHE Cess Amount")
                    column(ScessAmt; SHECessAmount)
                    {
                    }
                    ...
                    begin
                        ...
                        Clear(TDSTCSPer);
                        Clear(SHECessAmount);
                        Clear(TotalTDSTCSInclSHECESS);
                        TCSEntry.Reset();
                        TCSEntry.SetRange("Document No.", "Document No.");
                        if TCSEntry.FindSet() then begin
                            repeat
                                TotalTDSTCSInclSHECESS += TCSEntry."Total TCS Including SHE CESS";
                                SHECessAmount += TCSEntry."SHE Cess Amount";
                                TDSTCSPer += TCSEntry."TCS %";
                        ...
                        GrandTotal1 := GrandTotal + TotalGSTAmtLine("Document No.") + "Sales Invoice Header"."Insurance Amount" + "Sales Invoice Header"."Freight Amt" - "Sales Invoice Header"."Trade Discount" - "Sales Invoice Header"."Discount Amount";
                        ...
                        IF SalesInvoiceLine1.FIND('-') THEN BEGIN
                            /* TotalBed := TotalBed +
                                ...
                                "Sales Invoice Header"."Posting Date", "Sales Invoice Header"."Currency Code"),
                                  0, "Sales Invoice Header"."Currency Factor");//16225
*/
                            ...
                            TotalEcess := TotalEcess +
                                ...
                                CurrExchRate.ExchangeAmtFCYToLCY(
                                "Sales Invoice Header"."Posting Date", "Sales Invoice Header"."Currency Code",
                                  0, "Sales Invoice Header"."Currency Factor"),
                        ...
                        Value := ("Sales Invoice Line"."Unit Price" * Quantity); // RKS AFTER JHA SIR RECOMMENDATION// BuyersRatePerSqMtr
...
RecState.NEXT = 0;
                ...
                IF "Posting Date" >= 20140107D THEN BEGIN// 010714D
                    policy := NewPolicy
                END ELSE
                    policy := Text13704;
                ...
                //16225 N/F Funcation CurrReport.LANGUAGE := Language.GetLanguageID("Language Code");
                ...
                IF Customer.FIND('-') THEN BEGIN
                    /*16225 field N/F
                        ...
                        CustCommissionerate := Customer.Collectorate;*/
                ...
                IF SIL.FindSet() THEN BEGIN
                    REPEAT
                        if SIL.Type = SIL.Type::Item then
                            GrandTotal += SIL."Line Amount";//16225 Amount To customer

                        IF SIL."No." = '51157000' THEN
                            GLRoundingOFF += SIL."Line Amount";//16225 Amount To customer
                    UNTIL SIL.NEXT = 0;
                END;
                //RKS
                ...
                */
                ...
                IF Cust."GST Registration No." = '' THEN BEGIN
                    ...
                    IF NOT "Sales Invoice Header"."QR Code".HASVALUE THEN BEGIN
                        ...
                        CLEAR(APIManagement);
                        PaymentText := APIManagement.CreateB2CQRPaymentURL("Sales Invoice Header", FALSE);
                        //16225 APIManagement.CreateQRCode(PaymentText, TempBlob);
    ...
    var
        ...
        // PostedStrOrderLineDetails: Record "13798"; //16225 table N/F
        ...
        Text13707: Label '93000021180500000012';
        ...
        NewPolicy: Label 'Goods insured under policy 93000021180500000012 w.e.f. 19/05/18 the Goods in transit are insured under Marine insurance with The New India Assurance Co. Limited, LCBO, New Delhi';
        CurCaption: Text[30];
        OuputNos: Integer;
        Sno: Integer;
        rowstogen: Integer;
        ...
        TotalTDSTCSInclSHECESS: Decimal;
        ...
        AmountTOCustomer: Codeunit CalcAmttoVendor;
        OtherTaxes: Decimal;
        shiptosta: Text[20];
